...
from src.core.base_broker import BaseBroker
from src.core.base_consumer import BaseConsumer
from src.core.base_publisher import BasePublisher
from src.core.base_store import BaseStore
from src.enums import FuncTypeEnum
from src.settings import settings
from src.utils.module_loading import import_string
...
class BoostParams(BaseJsonAbleModel):
    ...
    function_timeout: typing.Union[
        int, float] = 0  # 超时秒数，函数运行超过这个时间，则自动杀死函数。为0是不限制。 谨慎使用,非必要别去设置超时时间,设置后性能会降低(因为需要把用户函数包装到另一个线单独的程中去运行),而且突然强制超时杀死运行中函数,可能会造成死锁.(例如用户函数在获得线程锁后突然杀死函数,别的线程再也无法获得锁了)
